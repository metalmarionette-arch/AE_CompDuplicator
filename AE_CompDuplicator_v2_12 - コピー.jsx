(function() {
    ...
    // ───────────────────────────────
    // ■ エッセンシャルプロパティの更新処理を改良
    function updateEssentialProperties(mapping) {
        try {
            // まず全てのコンポジションのエッセンシャルプロパティを収集
            var essentialPropsToUpdate = [];
            
            for (var i = 0; i < mapping.length; i++) {
                if (mapping[i].duplicate instanceof CompItem) {
                    var dupComp = mapping[i].duplicate;
                    collectEssentialProperties(dupComp, essentialPropsToUpdate);
                }
            }
            
            // 収集したエッセンシャルプロパティの参照を更新
            for (var j = 0; j < essentialPropsToUpdate.length; j++) {
                updateEssentialPropertyReference(essentialPropsToUpdate[j], mapping);
            }
            
        } catch (e) {
            // エッセンシャルプロパティの更新でエラーが発生した場合は警告を表示
            // alert("エッセンシャルプロパティの更新中にエラーが発生しました: " + e.toString());
        }
    }
    
    function collectEssentialProperties(comp, essentialPropsArray) {
        try {
            // レイヤーを走査してエッセンシャルプロパティを探す
            for (var i = 1; i <= comp.numLayers; i++) {
                var layer = comp.layer(i);
                collectEssentialPropertiesFromPropertyGroup(layer, essentialPropsArray, comp);
            }
        } catch (e) {
            // エラーが発生した場合はスキップ
        }
    ...
    function collectEssentialPropertiesFromPropertyGroup(propGroup, essentialPropsArray, parentComp) {
        try {
            if (propGroup.numProperties !== undefined) {
                for (var i = 1; i <= propGroup.numProperties; i++) {
                    var prop = propGroup.property(i);
                    
                    // エッセンシャルプロパティかどうかをチェック
                    if (isEssentialProperty(prop)) {
                        essentialPropsArray.push({
                            property: prop,
                            comp: parentComp,
                            originalSource: getPropertySourceInfo(prop)
                        });
                    }
                    
                    // 再帰的にチェック
                    if (prop.numProperties !== undefined && prop.numProperties > 0) {
                        collectEssentialPropertiesFromPropertyGroup(prop, essentialPropsArray, parentComp);
                    }
                }
            }
        } catch (e) {
    ...
    function isEssentialProperty(prop) {
        try {
            // エッセンシャルプロパティの判定方法
            // 1. エクスプレッションでコンポジション参照がある
            // 2. または Motion Graphics Template に登録されている
            
            if (prop.canSetExpression && prop.expression && prop.expression.length > 0) {
                // エクスプレッションにcomp()参照がある場合
                if (prop.expression.match(/comp\s*\(\s*["'][^"']*["']\s*\)/)) {
                    return true;
                }
            }
            
            // Motion Graphics Template のチェック（バージョン対応）
            try {
                var comp = getCompFromProperty(prop);
                if (comp && comp.motionGraphicsTemplate) {
                    var mgt = comp.motionGraphicsTemplate;
                    for (var i = 1; i <= mgt.numProperties; i++) {
                        var essentialProp = mgt.property(i);
                        if (essentialProp && isSameProperty(prop, essentialProp)) {
                            return true;
                        }
                    }
                }
    ...
    function getCompFromProperty(prop) {
        try {
            var current = prop;
            while (current.propertyGroup) {
                current = current.propertyGroup;
            ...
            // レイヤーレベルまで遡る
            if (current.containingComp) {
                return current.containingComp;
            }
    ...
    function isSameProperty(prop1, prop2) {
        try {
            // プロパティの同一性を確認（完全な比較は困難なので基本的なチェック）
            return prop1.matchName === prop2.matchName && 
    ...
    function getPropertySourceInfo(prop) {
        try {
            var info = {
                hasExpression: false,
                expression: "",
            ...
            if (prop.canSetExpression && prop.expression && prop.expression.length > 0) {
                ...
                // comp()参照を抽出
                var compMatches = prop.expression.match(/comp\s*\(\s*["']([^"']*)["']\s*\)/g);
                if (compMatches) {
                    for (var i = 0; i < compMatches.length; i++) {
                        var match = compMatches[i].match(/comp\s*\(\s*["']([^"']*)["']\s*\)/);
                        if (match && match[1]) {
                            info.referencedComps.push(match[1]);
                        }
                    }
                }
            }
            ...
            return { hasExpression: false, expression: "", referencedComps: [] };
        }
    }
    
    function updateEssentialPropertyReference(essentialPropInfo, mapping) {
        try {
            var prop = essentialPropInfo.property;
            var sourceInfo = essentialPropInfo.originalSource;
            
            if (!sourceInfo.hasExpression || sourceInfo.referencedComps.length === 0) {
                return;
            }
            
            var updatedExpression = sourceInfo.expression;
            var hasChanges = false;
            
            // 参照されているコンポジション名を更新
            for (var i = 0; i < sourceInfo.referencedComps.length; i++) {
                var originalCompName = sourceInfo.referencedComps[i];
                
                // マッピングから対応する複製コンポを探す
                for (var j = 0; j < mapping.length; j++) {
                    if (mapping[j].original instanceof CompItem && 
                        mapping[j].original.name === originalCompName) {
                        
                        var newCompName = mapping[j].duplicate.name;
                        
                        // エクスプレッション内の参照を更新
                        var regex = new RegExp('comp\\s*\\(\\s*["\']' + originalCompName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '["\']\\s*\\)', 'g');
                        var newRef = 'comp("' + newCompName + '")';
                        
                        updatedExpression = updatedExpression.replace(regex, newRef);
                        hasChanges = true;
                        break;
                    }
                }
            ...
            // エクスプレッションが更新された場合は適用
            if (hasChanges && updatedExpression !== prop.expression) {
    ...
    function collectCompAssets(mode, renameOptions) {
        ...
        //--------------------------------------------------
        // ■ コンポ選択モード：複製後の source／式／Essential Property 更新
        if (mode === "duplicate") {
            // ① レイヤーの source を置換
            for (var i = 0; i < mapping.length; i++) {
                if (mapping[i].duplicate instanceof CompItem) {
            ...
            // ② エクスプレッション更新 ＆ ③ Essential Property (Media Replacement) 更新
            for (var i = 0; i < mapping.length; i++) {
                if (mapping[i].duplicate instanceof CompItem) {
                    var dupComp = mapping[i].duplicate;
                    for (var j = 1; j <= dupComp.numLayers; j++) {
                        var tgt = dupComp.layer(j);

                        // — エクスプレッション更新 —
                        updateExpressionsInPropertyGroup(tgt, mapping);

                        // — Essential Property 更新 —
                        if (dupComp.essentialProperties) {
                            updateEssentialPropertyAlternateSources(
                                dupComp.essentialProperties,
                                mapping
                            );
                        }
                    }
